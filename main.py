...
def withdraw_method(call):
    ...
    if len(parts) == 1:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("✅ تأكيد السحب", callback_data=f"approve_withdraw_{user_id}"),
            types.InlineKeyboardButton("❌ رفض السحب", callback_data=f"reject_withdraw_{user_id}")
        )
